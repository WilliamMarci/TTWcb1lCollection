...
**Note**: This is a dedicated repository for the Wcb analysis, based on the original `NanoTTH` framework developed by H. Qu.
...
# get NanoTTH (and switch to the desired branch)
# git clone git@github.com:ttH-charm/NanoTTH.git PhysicsTools/NanoTTH -b dev/nn
git clone git@github.com:PKU-Hep-Group/NanoTTH-Wcb.git PhysicsTools/NanoTTH -b main
...
```bash
cd PhysicsTools/NanoTTH/run
```

##### Make trees for heavy flavour tagging (bb/cc) or top/W data/MC comparison and scale factor measurement:

```bash
./runTTHTrees.py -o /path/to/output
(--sample-dir custom_samples -i /eos/cms/store/cmst3/group/vhcc/NanoAOD/NanoAODv9-ParticleNetAK4/_YEAR_)
--tth-channel [0L|1L|2L|1L,2L] --year [2015|2016|2017|2018] -n 10
(--batch) (--type [data|mc|data,mc]) (--tth-do-syst)
(--condor-extras '+AccountingGroup = "group_u_CMST3.all"')
```

To submit the created jobs (unless `--batch` option is given):
...
- use `--tth-syst` to make the systematic trees
...
More options of `runPostProcessing.py` or `runTTHTrees.py` (a wrapper of `runPostProcessing.py`) can be found with `python runPostProcessing.py -h` or `python runTTHTrees.py -h`, e.g.,
