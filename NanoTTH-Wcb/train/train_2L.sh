...
weaver --data-train \
    ...
    --data-config data/ttH_2L_${model}.yaml --network-config $modelopts \
    --model-prefix training/ttH_2L/${model}/{auto}${suffix}/net \
    --num-workers 1 --fetch-step 0.2 $batchopts \
    --num-epochs 20 --gpus 0 \
    --optimizer ranger --log logs/ttH_2L_${model}_{auto}${suffix}.log \
