...
def list_remote_dir(server, path):
    try:
        command = ['xrdfs', server, 'ls', path]
        result = subprocess.run(command, capture_output=True, text=True)
        # print(result.stdout)
        return result.stdout.splitlines()
    except subprocess.CalledProcessError as e:
        print(f"Cannot access remote directory {path} on server {server}: {e}")
        return []

def walk_remote_dir(server, path):
    items = list_remote_dir(server, path)
    files = [item for item in items if item.endswith('.root')]
    dirs = [item for item in items if not item.endswith('.root')]
    yield path, dirs, files
    for dir in dirs:
        yield from walk_remote_dir(server, dir)
...
def create_metadata(args):
    ...
    server = "root://cceos.ihep.ac.cn:1094"

    # discover all the datasets
    if args.inputdir:
        # found_samples = os.listdir(args.inputdir)
        found_samples = list_remote_dir(server, args.inputdir)
        
    found_samples = [os.path.basename(path) for path in found_samples]
    samps_with_remote_files = set()
    for samp in samp_to_datasets:
        ...
        for dataset in samp_to_datasets[samp]:
            ...
            if False:
                # use remote files if given a dataset published on DAS
                if dataset0 is None:
                ...
                samps_with_remote_files.add(samp)
            else:
                # local datasets on EOS
                ...
                for dp, dn, filenames in walk_remote_dir(server, sampdir):
                    if 'failed' in dp:
                        continue
                    for f in filenames:
    ...
    for samp in md['samples']:
        ...
        # create jobs
        nfiles = 1 if samp in samps_with_remote_files else args.nfiles_per_job
        for idx, chunk in enumerate(get_chunks(md['inputfiles'][samp], nfiles)):
...
def run_add_weight(args):
    print('Merging output files in %s ...' % args.outputdir)
    if args.weight_file:
    ...
    for samp in md['samples']:
        ...
        if 'error' in log_lower or 'fail' in log_lower:
            logging.error(log)
        if p.returncode != 0:
            #try to merge files one by one to find the bad file
            try:
                print("[DEBUG] Trying to merge files one by one")
                filereg = '{outputdir}/pieces/{samp}_*_tree.root'.format(
                    outputdir=args.outputdir, samp=samp)
                import glob
                filelist = glob.glob(filereg)
                outfile_tmp = outfile + '.tmp.root'
                try:
                    os.remove(outfile_tmp)
                except:
                    print("[DEBUG] No tmp file to remove")
                    pass
                # Merge files iteratively: always merge the current output with the next N files
                N = 100  # Number of files to merge at each step, can be adjusted
                merge_files = list(filelist)
                merged = None
                for i in range(0, len(merge_files), N):
                    chunk = merge_files[i:i+N]
                    if merged is None:
                        # First chunk, just merge these files
                        cmd = 'haddnano.py {outfile_tmp} {inputs}'.format(
                            outfile_tmp=outfile_tmp, inputs=' '.join(chunk))
                    else:
                        # Merge previous result with next chunk
                        cmd = 'haddnano.py {outfile_tmp} {merged} {inputs}'.format(
                            outfile_tmp=outfile_tmp, merged=merged, inputs=' '.join(chunk))
                    print("\rMerging files: ", chunk, end="")
                    logging.debug('...' + cmd)
                    p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                    log = p.communicate()[0]
                    log_lower = log.lower().decode('utf-8')
                    if p.returncode != 0 or 'error' in log_lower or 'fail' in log_lower:
                        logging.error(log)
                        raise RuntimeError('Hadd failed during iterative merge!')
                    # Remove previous merged file if exists (except first step)
                    if merged is not None and os.path.exists(merged):
                        os.remove(merged)
                    merged = outfile_tmp
                    # Prepare for next step: move to a new tmp file if more chunks remain
                    if i + N < len(merge_files):
                        next_tmp = outfile_tmp + f".step{i//N+1}"
                        os.rename(outfile_tmp, next_tmp)
                        merged = next_tmp
                # Final output: move last merged file to outfile
                if merged != outfile:
                    shutil.move(merged, outfile)
                # os.rename(outfile_tmp, outfile)
                print("\n[DEBUG] Merging succeeded.")
